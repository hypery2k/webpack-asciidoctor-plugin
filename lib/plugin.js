...
class AsciiDocRevalPlugin {
	...
	processSections(html, srcPath, outputPath) {
		return new Promise((resolve, reject) => {
	...
	writeAsciiDocFile(srcFile, destinationFile) {
		try {
			...
			let result = asciidoctor.convert(fs.readFileSync(srcFile, { encoding: 'utf8' }), this.options.adocOptions);
	...
	apply(compiler) {
		...
		compiler.plugin('compilation', (compilation) => {
			...
			compilation.plugin('html-webpack-plugin-after-html-processing', (htmlPluginData, callback) => {
				return this.processSections(htmlPluginData.html, options.srcFolder, outputOptions.path)
					.then((html) => {
